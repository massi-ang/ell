...
pydantic_ltype_aware_cattr.register_unstructure_hook(
    np.ndarray,
    lambda arr: (
        {"content": serialize_image(PIL.Image.fromarray(arr)), "__limage": True}
        if arr.ndim == 3
        else (
            {
...
pydantic_ltype_aware_cattr.register_unstructure_hook(set, lambda s: list(sorted(s)))
pydantic_ltype_aware_cattr.register_unstructure_hook(
    frozenset, lambda s: list(sorted(s))
)


pydantic_ltype_aware_cattr.register_unstructure_hook(
    PIL.Image.Image, lambda obj: {"content": serialize_image(obj), "__limage": True}
)
...
pydantic_ltype_aware_cattr.register_unstructure_hook(_lstr, unstructure_lstr)

pydantic_ltype_aware_cattr.register_unstructure_hook(
    BaseModel, lambda obj: obj.model_dump(exclude_none=True, exclude_unset=True)
)
...
def compute_state_cache_key(ipstr, fn_closure):
    _global_free_vars_str = (
        f"{json.dumps(get_immutable_vars(fn_closure[2]), sort_keys=True, default=repr)}"
    )
    _free_vars_str = (
        f"{json.dumps(get_immutable_vars(fn_closure[3]), sort_keys=True, default=repr)}"
    )
    state_cache_key = hashlib.sha256(
        f"{ipstr}{_global_free_vars_str}{_free_vars_str}".encode("utf-8")
    ).hexdigest()
