...
from ell.util.verbosity import (
    ...
    *,
    ...
    if not client:
        ...
        if not client and not was_fallback:
            raise RuntimeError(
                _no_api_key_warning(model, _name, "", long=True, error=True)
            )
    ...
    with model_usage_logger_post_intermediate(
        _logging_color, call_result.actual_n
    ) as _logger:
        tracked_results, metadata = provider_class.process_response(
            call_result,
            _invocation_origin,
            _logger if config.verbose and not _exempt_from_tracking else None,
            tools,
        )
    ...
    return (
        (tracked_results[0] if len(tracked_results) == 1 else tracked_results),
        call_result.final_call_params,
        metadata,
    )
