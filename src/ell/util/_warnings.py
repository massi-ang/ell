...
def _no_api_key_warning(
    model, name, client_to_use: Optional[Any], long=False, error=False
):
    color = Fore.RED if error else Fore.LIGHTYELLOW_EX
    prefix = "ERROR" if error else "WARNING"
    # openai default
    client_to_use_name = (
        client_to_use.__class__.__name__ if (client_to_use) else "OpenAI"
    )
    client_to_use_module = (
        client_to_use.__class__.__module__ if (client_to_use) else "openai"
    )
    return (
        f"""{color}{prefix}: No API key found for model `{model}` used by LMP `{name}` using client `{client_to_use_name}`"""
        + (
            f""".
...
"""
            if long
            else " at time of definition. Can be okay if custom client specified later! https://docs.ell.so/core_concepts/models_and_api_clients.html "
        )
        + f"{Style.RESET_ALL}"
    )
...
{Style.RESET_ALL}"""
        ...
        elif (client_to_use := config.registry[model]) is None:
            logger.warning(
                _no_api_key_warning(model, fn.__name__, client_to_use, long=False)
            )
...
def _autocommit_warning():
    if config.get_client_for("gpt-4o-mini")[0] is None:
        logger.warning(
            f"{Fore.LIGHTYELLOW_EX}WARNING: Autocommit is enabled but no OpenAI client found for autocommit model 'gpt-4o-mini' (set your OpenAI API key). Commit messages will not be written.{Style.RESET_ALL}"
        )
        return True
    return False
